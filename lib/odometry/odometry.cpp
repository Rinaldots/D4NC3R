...
Odometry::Odometry():
    ...
    heading_(0.0)
{
    odom_msg_.header.frame_id = micro_ros_string_utilities_set(odom_msg_.header.frame_id, "odom_link");
...
void Odometry::update(float vel_dt, float linear_vel_x, float linear_vel_y, float angular_vel_z)
{
    ...
    odom_msg_.pose.pose.orientation.x = q[1];
    odom_msg_.pose.pose.orientation.y = q[2];
    odom_msg_.pose.pose.orientation.z = q[3];
    odom_msg_.pose.pose.orientation.w = q[0];
